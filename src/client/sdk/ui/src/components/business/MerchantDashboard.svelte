...
<div class="space-y-4">
  <!-- Resumen -->
  <div
    class="stats stats-horizontal shadow-xl w-full bg-neutral-900/60 backdrop-blur-xl border border-white/10"
  >
    <div class="stat place-items-center">
      <div
        class="stat-title text-[10px] uppercase font-bold tracking-widest text-neutral-500"
      >
  ...
  {:else}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each dishes as dish}
        <GlassCard
          class="bg-neutral-900/40 hover:shadow-orange-500/10 transition-all duration-300 hover:-translate-y-1"
        >
          <div class="p-4">
            <div class="flex items-start justify-between gap-2">
              <h3 class="font-bold text-neutral-100 text-sm leading-tight">
                {dish.name}
              </h3>
              <StatusBadge
            ...
            <div class="card-actions justify-end mt-2 flex gap-2">
              <button
                class="btn btn-xs btn-outline btn-warning border-orange-500/20 text-orange-400 hover:bg-orange-500/10"
                >{m.merchant_edit()}</button
              >
              <button
                class="btn btn-xs btn-outline btn-error border-rose-500/20 text-rose-400 hover:bg-rose-500/10"
