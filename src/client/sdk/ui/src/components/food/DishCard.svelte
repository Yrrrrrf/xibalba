...
>
  ...
  <figure class="relative overflow-hidden rounded-t-3xl h-48 bg-gradient-to-br {accent}">
    ...
    <div class="absolute bottom-0 inset-x-0 p-3
      ...
      <p class="text-white text-xs font-medium drop-shadow">🍽️ {dish.restaurant}</p>
    </div>
  </figure>

  <!-- ── BODY ────────────────────────────────────────────────── -->
  <div class="flex flex-col flex-1 p-4 gap-2">
    <h3 class="font-bold text-neutral-100 text-[15px] leading-snug line-clamp-2">{dish.name}</h3>
