<script lang="ts">
  const zones = [
    { id: 'z1', name: 'Zona Centro Histórico', emoji: '🏛️' },
    { id: 'z2', name: 'Zona Costera Norte', emoji: '🏖️' },
    { id: 'z3', name: 'Zona Estadio Principal', emoji: '🏟️' },
    { id: 'z4', name: 'Zona Mercado Tradicional', emoji: '🛖' },
    { id: 'z5', name: 'Zona Fan Fest', emoji: '🎉' },
  ];

  let selectedZone = $state('z1');

  const selected = $derived(zones.find((z) => z.id === selectedZone)!);
</script>
...
<div class="card bg-neutral-900/60 backdrop-blur-xl shadow-xl border border-white/10 mb-4">
  <div class="card-body p-4">
    ...
    <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
      ...
      >
        {#each zones as zone}
          <option value={zone.id} class="bg-neutral-900 text-white">{zone.emoji} {zone.name}</option>
        {/each}
      </select>

      <div class="badge bg-orange-500/10 text-orange-400 border border-orange-500/20 gap-1 py-3 px-3">
        <span class="text-base">{selected.emoji}</span>
