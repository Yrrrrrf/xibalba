<script lang="ts">
  ...
  const bgColor = $derived(variantBg[variant] ?? variantBg.primary);
</script>
...
>
  ...
  <div
    class="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r {gradient} opacity-70"
  ></div>
  ...
  <div
    class="absolute inset-0 bg-gradient-to-br {gradient} opacity-0
              group-hover:opacity-[0.04] transition-opacity duration-400 rounded-2xl"
  ></div>
</div>
