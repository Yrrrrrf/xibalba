<script lang="ts">
  ...
  const zonas = $derived([...new Set(data.map((d) => d.zona))]);
  ...
  const filtered = $derived(
    data.filter((c) => {
      const matchSearch =
        ...
        c.propietario.toLowerCase().includes(search.toLowerCase());
      const matchZona = !filterZona || c.zona === filterZona;
      ...
      <tbody>
        {#each filtered as c}
          <tr class="hover:bg-white/5 border-b border-white/5 last:border-0 transition-colors">
            ...
            <td>
              <div class="font-bold text-sm text-neutral-100">{c.nombre}</div>
            </td>
            <td class="hidden md:table-cell text-sm text-neutral-400">{c.propietario}</td>
            <td class="hidden sm:table-cell">
              <span class="badge bg-white/5 border border-white/10 text-neutral-400 badge-sm text-[9px] font-bold uppercase">{c.zona}</span>
            </td>
            <td class="text-xs text-neutral-400 italic">{c.categoria}</td>
            <td class="hidden lg:table-cell text-center font-bold text-white">{c.platillos}</td>
            <td class="font-extrabold text-orange-400 text-center">{c.ventas}</td>
            <td>
              <span class="badge {estadoBadge[c.estado]} badge-sm capitalize font-bold text-[9px] uppercase">{c.estado}</span>
            </td>
            <td>
              <div class="flex gap-1">
                <button class="btn btn-xs btn-ghost hover:bg-orange-500/10 text-orange-400" aria-label="Ver">👁️</button>
