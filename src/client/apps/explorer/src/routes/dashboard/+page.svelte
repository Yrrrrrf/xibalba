<script lang="ts">
  ...
  const stats = [
    { title: 'Platillos', value: dishStore.all.length, icon: '🍽️', variant: 'primary' as const, trend: 'neutral' as const },
    { title: 'Ventas', value: dishStore.all.reduce((acc: number, d: any) => acc + d.sales_count, 0), icon: '💰', variant: 'success' as const, trend: 'up' as const, trendValue: '+5%' },
  ];
</script>

<svelte:head>
  <title>Xibalbá — Dashboard</title>
</svelte:head>
...
<main class="container mx-auto px-4 pt-6 pb-10 max-w-5xl">
  <div class="mb-8">
    ...
    <p class="text-neutral-500 text-xs mt-1 font-medium italic">
      Gestiona tus platillos y revisa tu rendimiento
    </p>
  </div>
