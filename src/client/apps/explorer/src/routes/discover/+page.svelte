...
<svelte:head>
  <title>Xibalba Explorer — {m.nav_discover()}</title>
</svelte:head>
            ...
            <div class="flex flex-col gap-4">
              <div class="flex gap-3 overflow-x-auto pb-1 scrollbar-hide mask-fade-right">
                {#each CATEGORIES as cat}
                  ...
                  >
                    <div class="p-1.5 rounded-xl bg-white/10" style="color: {categoriaActiva === cat.key ? 'inherit' : cat.color}">
                       <svelte:component this={cat.icon} size={18} strokeWidth={3} />
                    </div>
                    <span>{cat.label}</span>
                  </button>
                {/each}
