  ...
  <div class="flex items-center justify-between mb-8">
    <div>
      ...
      <p class="text-neutral-500 text-xs mt-1 font-medium italic">
        Añade, edita o pausa tus platillos disponibles
      </p>
    </div>
    <a
      href="/menu/new"
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-12">
      ...
      <GlassCard class="p-4 group">
        ...
        <div class="mt-4 flex items-center justify-between">
          ...
          <div class="flex gap-2">
            <button class="btn btn-xs btn-ghost hover:bg-orange-500/10 text-orange-400">Editar</button>
            <button class="btn btn-xs btn-ghost hover:bg-rose-500/10 text-rose-400">Pausar</button>
          </div>
        </div>
      </GlassCard>
    {/each}
    ...
    {#if dishStore.all.length === 0}
      <div class="col-span-full text-center py-20 bg-neutral-900/40 rounded-3xl border border-dashed border-white/10">
        ...
        <p class="text-stone-400">No tienes platillos en tu menú</p>
        <a href="/menu/new" class="btn btn-link text-orange-500 mt-2">Crear mi primer platillo</a>
      </div>
    {/if}
  </div>
</main>
