...
use xibalba_domain::entities::user::{User, Role, Session};
use xibalba_domain::entities::inventory::{Item, Category, Location};
...
pub fn validate_sku(sku: &str) -> Result<(), JsValue> {
    ...
    let re = regex::Regex::new(r"^[A-Z]{3}-\d{4}$").map_err(|e| JsValue::from_str(&e.to_string()))?;
    if re.is_match(sku) {
        Ok(())
    } else {
        Err(JsValue::from_str("Invalid SKU format. Expected /^[A-Z]{3}-\\d{4}$/"))
    }
}
